...
export function AgoraView({ onComplete }: { onComplete: (result: AgoraResult) => void }) {
  const scenario = useMemo(() => agoraScenarios[listAgoraSessions().length % agoraScenarios.length], []);
...
export function ForumView({ onComplete }: { onComplete: (result: ForumResult) => void }) {
  const seed = useMemo(() => forumChallenges[listForumSessions().length % forumChallenges.length], []);
  const [answer, setAnswer] = useState("");
